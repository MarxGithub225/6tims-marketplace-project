...
function TablePagination({ noPadding = false, data, limit = 10, currentPage = 1, setCurrentPage, setLimit }: TablePaginationProps) {
  ...
  const handlePageChange = (page: number) => {
    if (page > 0 && page <= (data ? data?.totalPages : 0)) {
      setCurrentPage(page);
    }
  };

  const paginationItems = generatePaginationItems(currentPage, (data ? data?.totalPages : 0), handlePageChange);
  ...
  return <div className={`flex items-center justify-between w-full ${!noPadding ? 'otherWidth:px-6' : ''}`}>
    <div className="pagination flex items-center space-x-2.5">
      <div className={`pagination-preview flex items-center ${data?.hasPrevPage ? 'cursor-pointer' : 'cursor-not-allowed'}`}
      ...
      <div className={`pagination-next flex items-center ${data?.hasNextPage ? 'cursor-pointer' : 'cursor-not-allowed'}`}
        onClick={() => handlePageChange(currentPage + 1)}
      >
    ...
    <div className="hidden bigTablet:block pagination" ref={selectRef} >
      ...
      {openSelect && <div className="pagination-select text-center">
        {

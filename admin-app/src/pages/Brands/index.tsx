...
import { Brand, CreateRequest, PaginationOptionBrand } from '../../sdks/brand-v1/utils/DataSchemas'
...
export default function BrandPage() {
    ...
    const { client } = useBrand()

    const queryClient = useQueryClient()
    dispatch(setTitle('Marques'))
    ...
    const { data: settingData, isLoading, isFetching, isError }: any =
        useQuery({
            queryKey: ['brandsData', search, page, limit, selectedOption],
            queryFn: async () => {
                let filter: PaginationOptionBrand = {page, limit}
                if(search) {
                ...
                let result: Pagination<Brand> = await client.getAllBrands(filter)
                if(isSearching) {
    ...
    const upsertMutation = useMutation({
        mutationFn: async () => {
            return currentSetting &&
                (!!currentSetting?._id ?
                    await client.updateBrand(currentSetting._id, currentSetting) :
                    await client?.createBrand(currentSetting)
                )
        },
        onSuccess: () => {
            toggleModal()
            ...
            queryClient.invalidateQueries({ queryKey: ["brandsData"] }).catch(e => console.log(e))
        },
        onError: (e: any) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async () => await client?.deleteBrand(currentSetting?._id),
        onSuccess: () => {
    ...
    return (
        <div className="page-content flex justify-center car-settings-page">
                                    ...
                                    <div className="w-full">
                                        ...
                                        <div className="w-full otherWidth:pl-6 otherWidth:pr-6 overflow-auto mobile-scroll no-scrollbar">
                                            <table className='car-settings-table'>
                                                <thead>
                                                    <tr>
                                                        <th style={{ width: 650 }} className='text-left'>Name</th>
                                                ...
                                                {!isSearching && settingData?.length === 0 && <tr>
                                                    ...
                                                    {!isSearching && settingData?.map((row: Brand, key: number) => {
                                                        return <tr key={key}>
            ...
            <Suspense fallback={<div>...</div>}>
                {modalOpened &&
                    <AddSetting settingTitle={'Marque'}

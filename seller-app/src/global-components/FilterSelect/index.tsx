...
function FilterSelect({ label = '', selectedOption, options, classname = "status-filter", setSelectedOption, width }: FilterSelectProps) {
    ...
    return <div className={`relative ${classname}`} onClick={() => setFilterOptionOpen(!filterOptionsOpen)} style={{ width: width ? width : 'auto' }} >
        {selectedOption?.label ?? 'Choose a setting'} <Arrow className="ml-2 h-3 w-auto transform rotate-90" />

        {filterOptionsOpen && <div ref={optionsRef} className="absolute right-0 top-16 bigTablet:top-12 z-50" style={{ width: width ? width : 'auto' }}>

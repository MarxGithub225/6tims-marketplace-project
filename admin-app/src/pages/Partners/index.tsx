...
import { CreateRequest, Partner} from '../../sdks/partner-v1/utils/DataSchemas';
...
export default function PartnersPage() {
    ...
    const { client: clientPartner } = usePartner()
    ...
    const { data: usersData, isLoading, isFetching, isError }: any = useQuery({
        queryFn: async () => {
            ...
            let result: Pagination<Partner> = await clientPartner.getAllPartners(filter)
            if(isSearching) {
        ...
        queryKey: ['allPartners', search, page, limit, selectedOption],
    })
    ...
    const upsertMutation = useMutation({
        mutationFn: async () => {
            return currentSetting &&
                (!!currentSetting?._id ?
                    await clientPartner.updatePartner(currentSetting._id, currentSetting) :
                    await clientPartner?.createPartner(currentSetting)
                )
        },
        onSuccess: () => {
            toggleModal()
            ...
            queryClient.invalidateQueries({ queryKey: ["allPartners"] }).catch(e => console.log(e))
        },
        onError: (e: any) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async () => await clientPartner?.deletePartner(currentSetting?._id),
        onSuccess: () => {
                            ...
                            <div className="w-full otherWidth:px-6 flex justify-center overflow-auto mobile-scroll no-scrollbar">
                                <table className='user-table'>
                                    ...
                                    <tbody>
                                        ...
                                        {!isSearching && usersData?.map((row: any, key: number) => {
                                            return <tr key={key}>
                                                <td style={{ width: 100, height: 60 }}>
                                                    {
                                                        !row?.seller?.personnalInfo?.image?.path ?
                                                            <>
                                                                {/*<div style={{width: 100, height: 60, backgroundColor: "#cbd5e1"}}/>*/}
                                                                <div style={{ width: 80, height: 60, display: 'flex', alignItems: 'center' }}>--</div>
                                                            </>
                                                            :
                                                            <img src={`${API_FILE_URL}/icons/${row?.seller?.personnalInfo?.image?.path}`} width={80} height={60} style={{ width: 80, height: 60 }} className="overflow-hidden object-cover rounded-md" alt=""
                                                            />
                                                    }
                                                </td>
                                                <td>
                                                <Link to={`${LinksEnum.sellers}/${row?.seller?._id}`} className='capitalize line-clamp-1' style={{ width: 150, fontWeight: 700 }}>
                                                ...
                                                <td>
                                                    <p className='line-clamp-1' style={{ width: 150 }}>
                                                        {row?.seller?.personnalInfo?.number}
                                                    </p>
                                                </td>
                                                <td>
                                                    <p className='line-clamp-1' style={{ width: 150 }}>
                                                        {row?.seller?.locationInfo?.cityName}
                                                    </p>
                                                </td>
                                                <td className='text-center'>
                                                    <p className='line-clamp-1 text-center' style={{ width: 100 }}>
                                                        {row?.seller?.soldNumber}
                                                    </p>
                                                </td>
